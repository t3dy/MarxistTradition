(async function () {
  const [comedians, thinkers, terms] = await Promise.all([
    fetch('data/comedians.json').then(r => r.json()),
  ...
  comedians.forEach(a => (a.tags || []).forEach(t => allTags.add(t)));
  ...
  function render() {
    renderFilters();
    const filtered = comedians.filter(a => matchesSearch(a) && matchesTag(a));
    resultsEl.innerHTML = '';
    ...
    for (const a of filtered) {
      ...
      card.innerHTML = `
        ...
        <div class="meta">
          ${a.genre ? `<span class="region-badge">${a.genre}</span>` : ''}
        </div>
