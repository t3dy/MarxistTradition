(async function () {
  const [pundits, thinkers, terms] = await Promise.all([
    fetch('data/pundits.json').then(r => r.json()),
  ...
  pundits.forEach(a => (a.tags || []).forEach(t => allTags.add(t)));
  ...
  function render() {
    renderFilters();
    const filtered = pundits.filter(a => matchesSearch(a) && matchesTag(a));
    resultsEl.innerHTML = '';
    ...
    for (const a of filtered) {
      ...
      card.innerHTML = `
        ...
        <div class="meta">
          ${a.platform ? `<span class="region-badge">${a.platform}</span>` : ''}
        </div>
