(async function () {
  const [analyses, thinkers, terms] = await Promise.all([
  ...
  function render() {
    ...
    const filtered = analyses.filter(a => matchesSearch(a) && matchesTag(a));
    resultsEl.innerHTML = '';

    if (filtered.length === 0) {
      resultsEl.innerHTML = '<p style="color: var(--color-text-muted); grid-column: 1/-1;">No analyses found.</p>';
      return;
    }
